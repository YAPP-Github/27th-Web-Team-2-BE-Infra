...
- 비교 테스트를 위해 `route_primary_api_to_lambda=false`로 운영 `api.moit.kr`, `api.weddin.kr`를 ECS/ALB로 롤백했다. 적용 결과는 0 added, 2 changed, 4 destroyed였고, `api.*`용 API Gateway custom domain/mapping은 제거됐다.
